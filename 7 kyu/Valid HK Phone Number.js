...
// checks for a valid HK phone number
function isValidHKPhoneNumber(isvalid) {
	return (/^[0-9]{4} [0-9]{4}$/g).test(isvalid);
}

// checks for a valid HK number inside a string
function hasValidHKPhoneNumber(hasvalid) {
	return (/[0-9]{4} [0-9]{4}/g).test(hasvalid);
}
